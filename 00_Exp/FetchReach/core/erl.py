...
class HAC(object):
    def __init__(self, args, core):
        ...
        self.sub = Actor_Critic(state_dim=args.state_dim*2, action_dim=args.state_dim, gamma=args.gamma, tau=args.tau, buffer_size=args.buffer_size, is_mem_cuda=args.is_mem_cuda)
        self.sub_period = 50; self.success_criteria = 0.1
    ...
    def rollout(self, env, master, sub, master_noise=False, sub_noise=False, memorize=True):
        #Initializations
        ...
        while not done:
            ...
            #Process intrinsic reward for sub
            if memorize: #Sub
                temp_sub_buffer = self.fill_reward(temp_sub_buffer, sub_ir)
                for entry in temp_sub_buffer:
                    self.sub.replay_buffer.add_experience(entry[0], entry[1], entry[2], entry[3], entry[4], entry[5])

            ############## BACK TO MASTER ##############
            master_reward = sub_ir + master_env_reward #Count both the environment reward as well as the goal achieved reward
    ...
    def train(self):
        for episode in range(self.args.num_games):
