...
class HAC(object):
    def __init__(self, args, core):
        ...
        self.master = Actor_Critic(state_dim=args.state_dim, action_dim=args.goal_dim, gamma=args.gamma, tau=args.tau, buffer_size=args.buffer_size, is_mem_cuda=args.is_mem_cuda, out_act=None)
    ...
    def rollout(self, env, master, sub, master_noise=False, sub_noise=False, memorize=True, her_ratio=4):
        ...
        while not done:
            ...
            for time in range(self.sub_period):
                ...
                if memorize: sub_trajectory.append([sub_obs[0], subgoal, action, sub_next_obs[0], sub_step_reward, info]) #[state. goal, action, next_state, reward, done]
